...
{foreach from=$posts key=post_id item=post}
    <div class="post">
        <h1>
            <a href="{$smarty.const.SITE_DIR}index/post/{$post_id}">
                {$post.name}
            </a>
        </h1>
        <div class="postInfo">
            <div class="postDate">
                ...
                {foreach from=$post.rubrics item=rubric}{$rubric.rubric} {/foreach}
            </div>
        </div><!--postInfo-->
        <div class="cutImg">
            <a href="{$smarty.const.SITE_DIR}index/post/{$post_id}">
                <img class="bigImg" src="{$smarty.const.SITE_DIR}images/pictures/bigCut/{$post_id}.jpg" />
            </a>
        </div>
        <div class="postText">
            {$post.epilog}
        ...
        <div class="readMore">
            <a href="{$smarty.const.SITE_DIR}index/post/{$posy_id}">Читать дальше...</a>
        </div>
    </div><!--post-->
{/foreach}
<div class="pageList">
